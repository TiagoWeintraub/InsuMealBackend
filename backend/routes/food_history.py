...
@router.get("/")
def read_food_history(current_user=Depends(get_current_user), session: Session = Depends(get_session)):
    resource = FoodHistoryResource(session)
    return resource.get_all()

@router.put("/{food_history_id}")
def update_food_history(food_history_id: int, data: FoodHistoryUpdate, current_user=Depends(get_current_user), session: Session = Depends(get_session)):
    resource = FoodHistoryResource(session)
